...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (password.length < 4) {
      toast.error('Password must be at least 4 characters long.', {style :{
        background : "rgba(48, 46, 46, 1)",
        color : "white",
      }});
      setLoading(false);
    ...
    try {
      const response = await axios.post('/api/v1/auth/login',{email, password},{withCredentials :true, headers :{
