...
export async function POST(req: NextRequest, context: ContextType) {
  try {
    ...
    if (!existingLike) {
      return NextResponse.json({ message: "You didn't like this blog." }, { status: 400 });
    }

    const updateLike = await Like.findByIdAndDelete(existingLike._id);
    if (!updateLike) {
      return NextResponse.json({ message: "Something went wrong while updating Like." }, { status: 400 });
    }
